...
class Net(nn.Module):
    def __init__(self):
        ...
        self.convblock5 = nn.Sequential(
            nn.Conv2d(in_channels=16, out_channels=10, kernel_size=(1, 1), bias=False),  # output  RF: 28
        )
...
class Net2(nn.Module):
    def __init__(self, drop: float = 0.0):
        ...
        #GAP Layer
        self.gap = nn.Sequential(
            nn.AvgPool2d(5) # Global average pooling
        )
        #Fully Connected Layer
        # Fully connected layer
        self.conv4 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=10, kernel_size=(1, 1), padding=0, bias=False),  # output  RF: 28
        )
