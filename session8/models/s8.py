...
class Net(BaseNet):
    def __init__(self, drop: float=0, norm: str='bn', num_groups: int = 2):
        ...
        self.out = nn.Sequential(
            nn.AvgPool2d(6),
            nn.Conv2d(in_channels=48, out_channels=10, kernel_size=(1, 1), bias=False),  # output  RF: 28
        )
...
class Bottleneck(BaseNet):
    def __init__(self, in_plane: int, planes: List[int], norm: str, num_groups: int, drop: float):
